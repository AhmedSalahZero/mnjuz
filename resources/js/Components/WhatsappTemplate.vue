<script setup>
    ...
    import { mediaPreviewSource } from '@/Composables/templateMediaPreview';

    const props = defineProps({
        /**
         * true = ارسم العنصر النائب. الافتراضي true عن قصد: صفحة العرض
         * تمرّره صراحةً، ومن ينسى تمريره لا يرى صورة مكسورة.
         */
        placeholder: {
            type: Boolean,
            default: true
    ...
    /**
     * رابط الوسائط المعروض. المعامل يصل بأشكال ثلاثة (رفع/سابق/محفوظ)،
     * وتوحيدها في مكان واحد يمنع تكرار الاختيار في كل وسم أدناه.
     */
    const mediaSource = computed(() => mediaPreviewSource(props.parameters?.header?.parameters?.[0]));
...
<template>
    <div v-if="visible" class="mr-auto rounded-lg rounded-tl-none my-[0.1em] p-1 text-sm bg-white flex flex-col relative speech-bubble-left max-w-[25em]">
        <div v-if="parameters.header.format != null && parameters.header.format != 'TEXT'">
            <div v-if="placeholder || !mediaSource" class="bg-[#ccd0d5] mb-4 flex justify-center rounded py-8">
                        ...
                        <div class="flex space-x-2 w-full h-1/3 bg-white opacity-90 pt-2">
                            ...
                            <div>
                                ...
                                <div class="flex items-center text-slate-500 text-[11px]">
                                    <span>{{ getExtension(mediaSource) }}</span>
                                </div>
                            </div>
                            <a :href="mediaSource" @click="downloadClicked" class="flex justify-end w-full">
