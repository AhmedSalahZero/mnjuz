	...
	<div class="h-20 bg-white border-b border-1 md:flex items-center justify-between px-10 hidden">
		...
		<div>
			<Link v-if="!props.contact" href="/contacts"
				class="inline-flex justify-center rounded-md border border-transparent bg-slate-200 px-4 py-2 text-sm text-slate-500 hover:bg-slate-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 mr-4">
				{{ $t('Cancel') }}</Link>
		...
		<form @submit.prevent="submitForm()" class="w-[30em]">
			<div class="flex justify-center items-center">
				<div class="rounded-full w-40 h-40 m-4">
					<svg v-if="fileUrl === null" class="text-gray-500" viewBox="0 0 24 24" fill="currentColor"
						aria-hidden="true">
				...
				<label for="file-upload"
					class="cursor-pointer inline-flex justify-center rounded-md border border-transparent bg-slate-200 px-4 py-2 text-sm text-slate-700 hover:bg-slate-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 mr-4">{{ $t('Upload image') }}</label>
			</div>
			<div class="grid gap-x-6 gap-y-4 sm:grid-cols-6 pb-6 border-b">
				...
				<FormSelect v-model="form.group" :name="$t('Group')" :error="form.errors.group"
					:options="contactGroupOptions()" :type="'text'" :class="'sm:col-span-6'" :multiple="true" />
				<FormSelect v-if="contactCategoriesEnabled && (contactCategories || []).length"
					v-model="form.categories" :name="$t('Categories')" :error="form.errors.categories"
					:options="contactCategoryOptions()" :type="'text'" :class="'sm:col-span-6'" :multiple="true" />
			</div>
			<div v-if="locationSettings === 'before' && props.fields.length > 0">
				<div class="grid gap-x-6 gap-y-4 sm:grid-cols-2 mt-4 pb-6 border-b">
					<div v-for="(input, index) in props.fields" :key="index"
						...
						<FormInput v-if="input.type === 'input'" v-model="form.metadata[input.name]" :name="input.name"
							:label="$t(input.name)" :type="input.value" :class="'sm:col-span-2'"
							:required="input.required === 1 ? true : false" />
						<FormTextArea v-if="input.type === 'textarea'" v-model="form.metadata[input.name]"
							:name="input.name" :label="$t(input.name)" :class="'sm:col-span-2'"
							:required="input.required === 1 ? true : false" />
						<FormSelect v-if="input.type === 'select'" v-model="form.metadata[input.name]"
							:name="input.name" :options="transformOptions(input.value)" :class="'sm:col-span-2'"
							:required="input.required === 1 ? true : false" />
						<FormCheckbox v-if="input.type === 'checkbox'" v-model="form.metadata[input.name]"
							:name="input.name" :label="input.name" :class="'sm:col-span-2'"
							:required="input.required === 1 ? true : false" />
					</div>
				</div>
			</div>
			...
			<div class="mt-4 mb-10 pb-10 flex">
				<Link href="/contacts"
					...
					{{ $t('Cancel') }}</Link>
				<button
					:class="'inline-flex justify-center rounded-md border border-transparent bg-primary px-4 py-2 text-sm text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2'">
					<span>{{ $t('Save') }}</span>
				</button>
			</div>
		</form>
