	...
	<div :class="className">
		...
		<div class="mt-2">
			<textarea
				...
				:class="error ? 'ring-[#b91c1c]' : 'ring-gray-300'" :value="props.modelValue" @input="updateValue"
				:placeholder="placeholder" :rows="textAreaRows" ref="textareaRef">
			{{ props.modelValue }}
			</textarea>
		</div>
			...
			<button class="hover:bg-slate-100 rounded p-1" @click="format('monospace')">
				<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24">
					<path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
						stroke-width="1.5" d="M13.5 6L10 18.5m-3.5-10L3 12l3.5 3.5m11-7L21 12l-3.5 3.5" />
				</svg>
			</button>
			<button @click="addVariable()"
				class="hover:bg-slate-100 rounded p-1 text-sm">{{ $t('Add variable') }}</button>
		</div>
	</div>
	<div class="mt-3 bg-slate-100 p-3">
		<h1>{{ $t('Samples') }}</h1>
		<p class="text-xs">
			{{ $t('In order for your template to be reviewed properly, please add an example for each variable') }}
		</p>
		<div class="mt-4">
