...
const submitForm5 = async () => {
	...
	form5.put('/profile/verification', {
		preserveScroll: true,
		onFinish: () => {
			isLoading.value = false
		},
			...
			<div class="fixed inset-0 overflow-y-auto">
				<div class="flex min-h-full items-center justify-center p-4 text-center">
					<TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
						...
						<DialogPanel
							class="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all min-w-[20em]">
							<div>
								<h2 class="text-base text-xl leading-7 text-gray-900">{{ $t('Personal information') }}
								</h2>
								<p class="mb-4 text-sm leading-6 text-gray-600">
									{{ $t('Update your account details and credentials') }}
								</p>
								<TabGroup>
									...
									<TabPanels class="mt-2">
											...
											<div class="mt-5 grid grid-cols-1 gap-x-6 gap-y-4">
												<form @submit.prevent="submitForm()"
													...
													<FormPhoneInput v-model="form1.phone" :name="$t('Phone')"
														...
														:class="'sm:col-span-6'" />
													<FormSelect v-model="form1.language" :name="$t('Language')"
														:error="form1.errors.language" :options="languageOptions"
														:class="'sm:col-span-6'" :dropdownUp="true" />
													<div class="mt-4 flex">
										...
										<TabPanel v-if="tfa.status">
											<form @submit.prevent="submitForm4()">
												...
												<div v-if="tfa.enabled === 1" class="mt-2 text-sm text-green-700">
													Two-factor authentication is <b>enabled</b>. </div>
												<div class="relative mt-4">
													<div class="absolute inset-0 backdrop-blur-sm"
														v-if="form4.status === 0" />
													<h3 class="font-semibold">1. Scan QR Code</h3>
													<p class="text-gray-600 text-sm"> Open the authentication app (ex:
														Authy, Google Authenticator) on your mobile device and scan the
													...
													<p class="text-sm text-gray-600 mb-2"> To confirm that you setup
														your code properly, please enter the 6-digit token from your
														mobile app. </p>
										...
										<TabPanel v-if="tfa.status">
											<form @submit.prevent="submitForm5()">
												<FormSelect name="Verifications" v-model="form5.verification_enabled"
													:options="[
														{ value: 1, label: 'Enable' },
														{ value: 0, label: 'Disable' },
													]" :error="form5.errors.verification_enabled" />
												<div class="mt-2 text-sm text-slate-600">
													{{ $t('When enabled, you must verify your account once before dashboard access.') }}
												</div>
												<div class="mt-4 flex">
													<button type="button" @click="closeModal"
														class="inline-flex justify-center rounded-md border border-transparent bg-slate-50 px-4 py-2 text-sm text-slate-500 hover:bg-slate-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 mr-4">{{ $t('Cancel') }}</button>
