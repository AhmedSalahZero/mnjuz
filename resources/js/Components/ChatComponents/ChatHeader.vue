...
<template>
	...
	<div v-if="addon == 1 && !props.contact?.is_blocked"
		class="flex justify-between items-center bg-white border-b border-1 py-2 px-4 md:px-4">
		<div class="text-sm py-1 px-3 rounded-md flex items-center gap-x-2 w-[fit-content]"
		...
		<div class="flex items-center gap-x-2">
			...
			<div class="w-12 h-6 flex items-center bg-gray-300 rounded-full p-1 cursor-pointer"
				:class="{ 'bg-primary': form3.ai_assistant }" @click="submitForm3()">
	...
	<AlertModal v-model="showBlockAlert" :label="$t('Block Contact')" @confirm="blockContact(props.contact.id)"
		:description="$t('Are You sure you want to block this contact') +
			' [  ' +
			...
			' ]'
			" />
	<AlertModal v-model="showUnblockAlert" :label="$t('Unblock Contact')" @confirm="unblockContact(props.contact.id)"
		:description="$t('Are You sure you want to unblock this contact') +
			' [  ' +
			props.contact.first_name +
			' ' +
			props.contact.last_name +
