...
<template>
	...
	" @submit.prevent="sendMessage()" class="flex items-center px-2 md:px-10 space-x-2">
		<div class="flex items-center w-full rounded-lg py-4 md:py-2 pl-2 pr-2"
			...
			<textarea ref="textInputRef" @focus="form.type = 'text'" @keydown.enter.exact.prevent="handleEnterKey"
				...
				@input="adjustTextareaHeight" type="text" rows="1" :autofocus="true"
				:placeholder="$t('Type your message...')" :disabled="processingForm">
      </textarea>
			...
			<button class="flex items-center" type="submit"
				...
				<svg v-if="!processingForm" :class="formTextInput === null || formTextInput.trim() === '' ? 'text-slate-300' : 'text-black'
		...
		<div class="bg-white rounded-md w-full p-4">
			<div>
				<textarea ref="textInputRef" @focus="form.type = 'text'" @keydown.enter.exact.prevent="handleEnterKey"
					...
					@input="adjustTextareaHeight" type="text" rows="3" :autofocus="true"
					:placeholder="$t('Type your message...')" :disabled="processingForm">
        </textarea>
				<input type="file" class="sr-only" :accept="getAcceptedFileTypes()" id="file-upload"
					@change="handleFileUpload($event)" />
			...
			<div class="flex gap-x-4 items-center text-gray-600">
				...
				<div class="ml-auto">
					<button class="flex items-center gap-x-2 p-1 px-2 rounded-md border border-gray-400 text-sm"
						...
						<span :class="(formTextInput === null || formTextInput.trim() === '') && !form2.file
							? 'text-slate-300'
							: 'text-black'
							">Send</span>
						<div>
							<svg v-if="!processingForm" :class="(formTextInput === null || formTextInput.trim() === '') && !form2.file
								? 'text-slate-300'
								: 'text-black'
