...
<template>
  <div
    ...
    <div>
      ...
      <div
        v-if="JSON.parse(content.metadata).type === 'text'"
        ...
        <div
          v-if="JSON.parse(content.metadata)?.buttons"
          class="mr-auto text-sm text-[#00a5f4] flex flex-col relative max-w-[25em]">
          <div
            v-for="(item, index) in JSON.parse(content.metadata)?.buttons"
            :key="index"
      ...
      <div
        v-if="JSON.parse(content.metadata).type === 'unsupported'"
        style="overflow-wrap: break-word"
        class="max-w-[320px]">
      ...
      <div
        v-if="JSON.parse(content.metadata).type === 'button'"
        style="overflow-wrap: break-word"
        class="max-w-[320px]">
        <p class="normal-case whitespace-pre-wrap">
      ...
      <div
        v-if="JSON.parse(content.metadata).type === 'interactive'"
        style="overflow-wrap: break-word"
        class="max-w-[320px]">
        ...
        <p
          v-if="JSON.parse(content.metadata).interactive.type == 'list_reply'"
      ...
      <div v-else-if="JSON.parse(content.metadata).type === 'image'">
        <img
          v-if="content.media != null"
          :src="content?.media?.path"
          alt="Image"
          class="mb-2 max-w-[320px]" />
        <div
          v-else
          class="text-slate-500 flex justify-center items-center space-x-4">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            ...
            <g fill="none">
              ...
              <path
                fill="currentColor"
                d="m13.299 3.148l8.634 14.954a1.5 1.5 0 0 1-1.299 2.25H3.366a1.5 1.5 0 0 1-1.299-2.25l8.634-14.954c.577-1 2.02-1 2.598 0ZM12 4.898L4.232 18.352h15.536L12 4.898ZM12 15a1 1 0 1 1 0 2a1 1 0 0 1 0-2Zm0-7a1 1 0 0 1 1 1v4a1 1 0 1 1-2 0V9a1 1 0 0 1 1-1Z" />
            </g>
          </svg>
          {{ $t('Content not available') }}
        </div>
        <div
          v-if="JSON.parse(content.metadata).image?.caption"
          style="overflow-wrap: break-word"
          class="max-w-[320px]">
      ...
      <div v-else-if="JSON.parse(content.metadata).type === 'document'">
        <div class="relative w-[300px]">
          <div class="flex space-x-2 w-full h-1/3 bg-white opacity-90 pt-2">
            <div>
              <svg
                v-if="getExtension(content?.media?.type) === 'PDF'"
                xmlns="http://www.w3.org/2000/svg"
                width="36"
                height="36"
              ...
              <svg
                v-if="
                  getExtension(content?.media?.type) === 'XLSX' ||
                  getExtension(content?.media?.type) === 'XLS'
                "
                xmlns="http://www.w3.org/2000/svg"
                width="36"
                height="36"
              ...
              <svg
                v-if="
                  getExtension(content?.media?.type) === 'DOC' ||
                  getExtension(content?.media?.type) === 'DOCX'
                "
                xmlns="http://www.w3.org/2000/svg"
                width="36"
                height="36"
              ...
              <svg
                v-if="
                  getExtension(content?.media?.type) === 'PPT' ||
                  getExtension(content?.media?.type) === 'PPTX'
                "
                xmlns="http://www.w3.org/2000/svg"
                width="36"
                height="36"
            ...
            <div>
              <div>
                <p class="overflow-ellipsis overflow-hidden line-clamp-2 w-[200px]">
                  {{ content?.media?.name }}
                </p>
              </div>
            ...
            <a
              :href="content?.media?.path"
              :download="content?.media?.name"
              target="_blank"
      ...
      <div v-else-if="JSON.parse(content.metadata).type === 'location'">
        ...
        <!-- زر الاتجاهات -->
        <button
          @click="openInGoogleMaps(content.metadata)"
      ...
      <div v-else-if="JSON.parse(content.metadata).type === 'video'">
        ...
        <div
          v-if="JSON.parse(content.metadata).video?.caption"
          style="overflow-wrap: break-word"
          class="max-w-[320px]">
      ...
      <!--Timestamp-->
      <div
        v-if="props.type === 'outbound' && content.user"
        class="mt-2 mb--2">
        <span class="text-gray-500 text-xs text-right leading-none"
          >Sent By: <u>{{ content.user?.first_name + ' ' + content.user?.last_name }}</u></span
        >
      </div>
      <div
        ...
        <span
          ...
          <svg
            v-if="chatStatus(content.logs) === 'sent'"
            xmlns="http://www.w3.org/2000/svg"
            width="16"
            height="16"
            viewBox="0 0 16 16">
          ...
          <svg
            v-if="chatStatus(content.logs) === 'delivered' || chatStatus(content.logs) === 'read'"
            xmlns="http://www.w3.org/2000/svg"
            width="18"
            height="18"
            viewBox="0 0 16 16">
          ...
          <svg
            ...
            v-if="chatStatus(content.logs) === 'failed'"
            class="text-red-600"
            xmlns="http://www.w3.org/2000/svg"
            width="16"
            height="16"
      ...
      <div
        v-if="JSON.parse(content.metadata).type === 'contacts'"
  ...
  <Modal
    :label="$t('Message status: ') + chatStatus(content.logs)"
    :isOpen="isModalOpen"
    :closeBtn="true"
    ...
    <div>
      <div
        v-if="errors.length"
        class="bg-red-100 rounded-md p-3 text-sm mt-4">
        <div
          ...
          <div class="flex">
            ...
            <button
              @click="copyItem(JSON.parse(content.logs[0].metadata).id)"
      ...
      <div class="mt-4 flex w-full">
        <button
          ...
          class="w-full justify-center rounded-md border border-transparent bg-slate-50 px-4 py-2 text-sm text-slate-500 hover:bg-slate-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2">
          {{ $t('Cancel') }}
        </button>
      </div>
    </div>
