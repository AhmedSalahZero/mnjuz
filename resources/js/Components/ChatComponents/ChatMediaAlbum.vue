...
/** الفيديو الذي يعمل الآن — بلاطة واحدة في كل مرّة كي لا يتزاحم الصوت. */
...
<template>
	...
	>
		<div class="grid gap-1" :class="columns">
			...
			>
				<!-- الفيديو يُشغَّل في مكانه: معاينة الصور وسمُها <img> فلا تعرضه -->
				<video
					v-if="tileSource(message) && tileType(message) === 'video' && playingId === message.id"
					:src="tileSource(message)"
					class="h-[150px] w-full bg-black object-contain"
					controls
					autoplay
					@error="markBroken(message.id)"
				/>
				<div
					v-else-if="tileSource(message) && tileType(message) === 'video'"
					class="relative h-[150px] w-full cursor-pointer bg-black"
					@click="play(message)"
				>
					<video
						:src="tileSource(message)"
		...
		<div class="flex items-center justify-between space-x-4" :class="type === 'outbound' && last.user ? '' : 'mt-2'">
			<p class="text-gray-500 text-xs leading-none">
				{{ $t('{count} files', { count: messages.length }) }}
			</p>
