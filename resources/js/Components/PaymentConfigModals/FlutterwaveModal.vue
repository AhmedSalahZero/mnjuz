<script setup>
    ...
    const submitForm = async () => {
        form.put('/admin/payment-gateways/flutterwave', {
            preserveScroll: true,
            onSuccess: () => {
                onClose()
            },
    ...
    function getRow() {
        axios.get('/admin/payment-gateways/flutterwave').then((response) => {
...
<template>
    <Modal :label="$t('Edit Flutterwave configuration')" :isOpen=modelValue>
        <div class="mt-5 grid grid-cols-1 gap-x-6 gap-y-4">
            <form @submit.prevent="submitForm()" class="">

                <div class="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-6 sm:col-span-4 pb-8 border-b">
                    <FormInput v-model="form.public_key" :name="$t('Public key')" :type="'text'" :error="form.errors.public_key" :class="'sm:col-span-6'"/>
                    <FormInput v-model="form.secret_key" :name="$t('Secret key')" :type="'password'" :placeholder="secretSet.secret_key ? '•••••••••• ' + $t('Leave blank to keep current value') : ''" :error="form.errors.secret_key" :class="'sm:col-span-6'"/>
