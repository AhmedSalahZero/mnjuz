<script setup>
    ...
    import RoleTransferModal from '@/Components/Modals/RoleTransferModal.vue';
    ...
    const selectedRoleUuid = ref(null);

    const deleteAction = (key) => {
        selectedRoleUuid.value = key;
            ...
            <TableBodyRow v-for="(item, index) in rows.data" :key="index" :class="!isLastRow(index) ? 'border-b' : ''">
                ...
                <TableBodyRowItem :position="'last'">
                    <Dropdown :align="'right'" class="mt-2">
                        ...
                        <template #items>
                            <DropdownItemGroup>
                                <DropdownItem :href="'/admin/team/roles/' + item.uuid">{{ $t('View/edit') }}</DropdownItem>
