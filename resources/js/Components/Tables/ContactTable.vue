...
<template>
		...
		<div class="flex items-center space-x-2">
			<label @click="toggleAllCheckboxes($event)" for="myCheckbox" class="cursor-pointer">
				<div class="w-4 h-4 border border-gray-400 rounded-md flex items-center justify-center"
					:class="{ 'bg-secondary': bulkCheckbox }">
					<svg v-if="bulkCheckbox" class="w-4 h-4 text-white" fill="none" stroke="currentColor"
		...
		<div>
			<div class="float-right">
				<Dropdown :align="'right'" class="mt-2">
					<button
						class="inline-flex w-full justify-center rounded-md text-sm font-medium text-black hbg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75">
					...
					<template #items>
						<DropdownItemGroup>
							...
							<DropdownItem as="button" @click="isExportModalOpen = true">{{ $t('Export') }}
							</DropdownItem>
							<DropdownItem v-if="selectedCount > 0" as="button" @click="deleteItems()">
								{{ $t('Delete selected') }}</DropdownItem>
	...
	<div class="h-[5vh]">
		<div class="flex justify-between text-sm border-b">
			<Link href="/contacts" class="pt-3 flex-1 text-center pb-1 hover:bg-slate-50"
				:class="{ 'bg-gray-50 border-b-2 border-slate-700': $page.url.startsWith('/contacts') && !$page.url.startsWith('/contact-groups') && !$page.url.startsWith('/contact-categories') }">
				{{ $t('All contacts') }}
			</Link>
			<Link href="/contact-groups" class="pt-3 flex-1 text-center pb-1 hover:bg-slate-50"
				:class="{ 'bg-gray-50 border-b-2 border-slate-700': $page.url.startsWith('/contact-groups') }">
				{{ $t('Groups') }}
			</Link>
			<Link v-if="props.contactCategoriesEnabled" href="/contact-categories"
				class="pt-3 flex-1 text-center pb-1 hover:bg-slate-50"
				:class="{ 'bg-gray-50 border-b-2 border-slate-700': $page.url.startsWith('/contact-categories') }">
				{{ $t('Categories') }}
			</Link>
		</div>
	</div>
	<div class="flex-grow overflow-y-auto h-[65vh]" ref="scrollContainer">
		<div v-if="type === 'contact'" @click="getRow(contact.uuid)"
			class="flex space-x-2 hover:bg-gray-50 cursor-pointer px-4 py-3 border-b"
			:class="contact.isChecked ? 'bg-gray-50' : ''" v-for="(contact, index) in rows.data" :key="index">
			<div>
				<label @click.stop="toggleCheckbox(contact.uuid)" for="myCheckbox" class="cursor-pointer">
					<div class="w-4 h-4 border border-gray-400 rounded-md flex items-center justify-center mt-1"
						:class="{ 'bg-secondary': contact.isChecked }">
						<svg v-if="contact.isChecked" class="w-4 h-4 text-white" fill="none" stroke="currentColor"
							viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
			...
			<div class="w-[15%]">
				...
				<div v-else
					class="rounded-full bg-secondary/10 text-secondary flex justify-center items-center h-12 w-12">
			...
			<div class="w-[10%]">
				<svg v-if="contact.is_favorite" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
					viewBox="0 0 24 24">
					<path fill="#FFD700"
		...
		<div v-else-if="type === 'group' || type === 'category'" @click="getRow(row.uuid)"
			class="flex space-x-2 hover:bg-gray-50 cursor-pointer px-4 py-3 border-b"
			:class="row.isChecked ? 'bg-gray-50' : ''" v-for="(row, key) in rows.data" :key="key">
			<div>
				<label @click.stop="toggleCheckbox(row.uuid)" for="myCheckbox" class="cursor-pointer">
					<div class="w-4 h-4 border border-gray-400 rounded-md flex items-center justify-center mt-1"
						:class="{ 'bg-secondary': row.isChecked }">
						<svg v-if="row.isChecked" class="w-4 h-4 text-white" fill="none" stroke="currentColor"
							viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7">
							</path>
						</svg>
