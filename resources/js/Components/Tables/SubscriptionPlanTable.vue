...
<template>
    <div class="md:bg-white flex items-center border border-primary md:border-none md:shadow-sm h-12 md:h-10 w-full md:w-80 rounded-[0.5rem] mb-6 text-xl md:text-sm">
        ...
        <input @input="search" v-model="params.search" type="text" class="outline-none px-4 w-full" :placeholder="$t('Search plans')">
    ...
    <Table :rows="rows">
        <TableHeader>
            <TableHeaderRow>
                ...
                <TableHeaderRowItem>
                    <span class="float-right">{{ $t('Status') }}</span>
                </TableHeaderRowItem>
        ...
        <TableBody>
            <TableBodyRow v-for="(item, index) in rows.data" :key="index" :class="!isLastRow(index) ? 'border-b' : ''">
                ...
                <TableBodyRowItem class="capitalize">
                    <span class="py-1 float-right rounded-[5px] text-xs px-3 bg-[#ddebf7] text-slate-700">{{ $t(item.status) }}</span>
                </TableBodyRowItem>
