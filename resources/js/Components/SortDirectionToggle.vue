...
<template>
	<div class="text-sm">
		<span @click="toggleDropdown()" class="cursor-pointer hover:bg-slate-50 p-1 rounded-full sort-dd block">
			<svg v-if="direction == 'desc'" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
				viewBox="0 0 16 16">
		...
		<div v-if="isOpen" class="absolute bg-white z-10 px-1 py-2 mt-2 shadow w-[7em] rounded-md text-black">
			<div>
				<div @click="sort('desc')" class="block px-2 py-1 cursor-pointer hover:bg-slate-100 rounded-md"> Newest
				</div>
				<div @click="sort('asc')" class="block px-2 py-1 cursor-pointer hover:bg-slate-100 rounded-md"> Oldest
				</div>
			</div>
		</div>
	</div>
