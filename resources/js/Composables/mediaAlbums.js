/**
 * تجميع الوسائط المرسَلة دفعةً واحدة في فقاعة واحدة — كما يفعل واتساب.
 *
 * السحب والإفلات يرسل كل ملف رسالةً مستقلّة (وواجهة واتساب السحابية لا
 ...
 *
 * ما يجمعه: صورٌ وفيديوهات متتالية، بالاتجاه نفسه، غير محذوفة، بين كل
 * واحدة والتي تليها فجوة قصيرة. المستند والصوت يقطعان المجموعة كما يقطعها
 * النصّ — وهو سلوك واتساب نفسه: الألبوم صورٌ وفيديو لا مستندات.
 */

/** أنواع الوسائط التي تدخل الألبوم. */
export const MEDIA_ALBUM_TYPES = ['image', 'video']
...
const isAlbumCandidate = (entry) => {
    ...
    return MEDIA_ALBUM_TYPES.includes(parseMetadata(value.metadata).type)
}
 ...
 */
export function groupMediaAlbums(chats) {
...
export function albumCaption(messages) {
    for (const message of messages ?? []) {
        ...
        const caption = captionOf(meta[meta.type]?.caption)
...
/**
 * نوع البلاطة: image أو video. الشبكة ترسم لكلٍّ شكله — الفيديو بعلامة
 * تشغيل، ولا يُفتح في معاينة الصور لأنها <img> لا مشغّل.
 */
export function tileType(message) {
    const type = parseMetadata(message?.metadata).type

    return MEDIA_ALBUM_TYPES.includes(type) ? type : 'image'
}
