			...
			<div class="mt-20">
				...
				<div v-if="step === null" class="md:w-[20em] p-2">
					<form v-if="!isValidPurchaseCode" @submit.prevent="validateCode()">
						...
						<div v-if="purchaseCodeError != null"
							class="mt-4 p-2 bg-slate-200 border-l-[2px] border-red-500 my-4 max-w-[28em] text-sm">
							{{ purchaseCodeError }}</div>
						<FormInput v-model="form4.purchase_code" :name="$t('Purchase Code')" :type="'text'"
							:placeholder="'Purchase code'" :class="'sm:col-span-6 mb-2'" />
						<div class="mt-2">
							<button v-if="!isLoading" href="/install/database" type="submit"
								class="mt-4 rounded-md bg-primary px-3 py-2 text-sm text-white shadow-sm w-full">{{ $t('Next Step') }}</button>
							<button v-else type="button"
								class="mt-4 rounded-md bg-primary px-3 py-2 text-sm text-white shadow-sm w-full flex justify-center">
				...
				<div v-else-if="step === 'migrate'" class="md:w-[30em] p-2">
					<div class="bg-slate-200 p-2 border-l-[3px] border-red-800 text-sm">
						<p>{{ $t('When you click proceed, your script will be updated automatically! Do not navigate away from the page while this process is ongoing.') }}
						</p>
					</div>
					<form @submit.prevent="runMigration()">
						<div class="flex justify-center gap-x-2">
							<button v-if="!isLoading" type="submit"
								class="w-[10em] mt-4 rounded-md bg-primary px-3 py-2 text-sm text-white shadow-sm">{{ $t('Proceed') }}</button>
							<button v-else type="button"
								class="w-[10em] mt-4 rounded-md bg-primary px-3 py-2 text-sm text-white shadow-sm flex justify-center">
								<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
									<path fill="currentColor"
										d="M12 2A10 10 0 1 0 22 12A10 10 0 0 0 12 2Zm0 18a8 8 0 1 1 8-8A8 8 0 0 1 12 20Z"
										opacity=".5" />
				...
				<div v-else-if="step === 'finish'" class="md:w-[30em] p-2">
					...
					<div class="flex justify-between gap-x-2">
						<Link href="/"
							class="mt-4 rounded-md bg-primary px-3 py-2 text-sm text-white shadow-sm w-full text-center">
							{{ $t('Proceed To Main Site') }}</Link>
					</div>
				</div>
			</div>
		</div>
