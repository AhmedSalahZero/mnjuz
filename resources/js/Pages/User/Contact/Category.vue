<template>
    <AppLayout>
        <div class="bg-white md:bg-inherit md:flex md:flex-grow capitalize">
            <div class="md:w-[30%] md:flex flex-col h-full bg-white border-r border-l" :class="category ? 'hidden' : ''">
                <div class="px-4 pt-4">
                    <div class="flex justify-between mt-2">
                        <div class="flex space-x-1 text-xl">
                            <h2>{{ $t('Contact Categories') }}</h2>
                ...
                <ContactTable :rows="props.rows" :filters="props.filters" :type="'category'" :contactCategoriesEnabled="true" @callback="handleCategory"/>
            </div>
                ...
                <div v-else class="md:block pt-20 hidden">
                    <div class="border py-10 w-[30em] rounded-xl bg-white">
                        ...
                        <div class="flex justify-center mt-6">
                            <button @click="openModal()" class="bg-primary rounded-lg text-sm text-white p-2 px-8 text-center capitalize">{{ $t('Add category') }}</button>
                        </div>
                    </div>
                </div>
            </div>
