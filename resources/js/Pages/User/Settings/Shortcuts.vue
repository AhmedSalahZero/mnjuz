          ...
          <div class="bg-white border border-slate-200 rounded-lg text-sm mb-20 px-4 py-4">
            ...
            <div v-for="(row, index) in rows" :key="index"
              ...
              <div class="md:col-span-3">
                ...
                <div class="flex items-center rounded-md border px-2">
                  <span class="text-slate-400">/</span>
                  <input v-model="row.command" type="text" :placeholder="$t('e.g. hello')"
                    class="w-full outline-none px-1 py-2 text-sm" />
                </div>
              </div>
              <div class="md:col-span-6">
                ...
                <textarea v-model="row.message" rows="2" :placeholder="$t('Reply text...')"
                  class="w-full rounded-md border px-2 py-2 text-sm outline-none resize-none"></textarea>
              </div>
              <div class="md:col-span-2">
                <label class="block text-xs text-slate-500 mb-1">{{ $t('Visibility') }}</label>
                <select v-model="row.scope" :disabled="!canManageCompany && row.scope !== 'company'"
                  class="w-full rounded-md border px-2 py-2 text-sm outline-none bg-white">
              ...
              <div class="md:col-span-1 flex md:justify-center md:pt-6">
                <button type="button" @click="removeRow(index)" :title="$t('Delete')"
...
const save = () => {
  form.shortcuts = rows.value
    .filter((r) => r.command && r.command.trim() !== '' && r.message && r.message.trim() !== '')
    .map((r) => ({
      id: r.id,
