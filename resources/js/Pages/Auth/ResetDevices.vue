        ...
        <div class="flex h-screen justify-center">
            <div class="flex justify-center">
                <div class="w-[20em] mt-40">
                    ...
                    <h1 class="text-2xl text-center">{{ $t('Reset Linked Devices') }}</h1>
                    <p class="text-center text-sm text-slate-500 mt-2">{{ $t('Reset devices help text') }}</p>
                    ...
                    <form @submit.prevent="submitForm()" class="mt-5">
                        <div v-if="props.flash?.status?.message" class="text-sm bg-emerald-600 text-white mb-2 py-2 px-1 text-center rounded">
                            <span>{{ props.flash?.status?.message }}</span>
                        </div>
                        <FormInput v-model="form.email" :name="$t('Email')" :error="form.errors.email" :type="'email'" :class="'sm:col-span-6'"/>
                        <div v-if="form.errors.recaptcha_response" class="form-error text-[#b91c1c] text-xs">{{ form.errors.recaptcha_response }}</div>
                        <div class="mt-6">
                            <button v-if="!isLoading" type="submit" class="rounded-md bg-primary px-3 py-3 text-sm text-white shadow-sm w-full">{{ $t('Send reset link') }}</button>
