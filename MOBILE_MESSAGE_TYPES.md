...
- ينشئ جهة اتصال `+966500000000` ويضع عليها **56 رسالة** تغطّي الـ14 نوعاً + الحالات الحافّة + حالات الألبوم.
...
### حالات الألبوم في آخر المحادثة

آخر جزء من المحادثة مخصص للألبوم (القسم 9). ستة مواضع يجب أن تُجمع، وثلاثة يجب ألا تُجمع:

| الحالة | المتوقَّع |
|---|---|
| صورتان بينهما 4 ثوانٍ | ألبوم — أصغر مجموعة ممكنة |
| خمس صور | **أربع بلاطات و«+1»** على الرابعة |
| صورة + فيديو + صورة | ألبوم واحد. الفيديو بلاطة عليها علامة تشغيل، ولا يُفتح في معاينة الصور |
| ألبوم فيه رسالة `failed` | الحالة المجمّعة **`failed`** — الأدنى يحكم |
| ألبوم فيه صورة بلا `media` | بلاطة «المحتوى غير متاح» **تبقى داخل المجموعة** |
| صورتان واردتان | ألبوم — الاتجاه لا يمنع الضم، لكن الوارد لا يُضم إلى الصادر |
| صورة + مستند + صورة | **ثلاث فقاعات** — المستند يقطع المجموعة |
| صورتان بينهما 3 دقائق | **فقاعتان** — الفجوة أطول من 60 ثانية |
| مستند كبير (`catalog-2026.pdf`) | يمثل ملفًا رُفع عبر chunks. الشكل الواصل واحد سواء رُفع في request واحد أو على قطع |

التعليق يوضع على أول رسالة في المجموعة فقط، وهو ما يظهر تحت الألبوم كله.
...
## 9) عرض الصور المرسَلة دفعةً واحدة (ألبوم)

**لا شيء تغيّر في الـAPI.** لا حقل جديد ولا شكل جديد؛ التطبيق يعمل كما هو بلا أي تعديل. هذا القسم لمن أراد أن يعرضها شبكةً واحدة كما صار الداشبورد يفعل.

### لماذا لا يوجد «معرّف دفعة»

واجهة واتساب السحابية **لا تعرف الألبوم**: كل صورة رسالة مستقلّة، ولا شيء في ردّها يقول إنها أُرسلت مع غيرها. الألبوم في تطبيق واتساب نفسه **تجميعٌ عند العرض** لرسائل منفصلة. فالضمّ استنتاجٌ من الرسائل لا حقلٌ يصل.

### قاعدة الضمّ — كما ينفّذها الداشبورد حرفياً

تُضمّ رسالتان متتاليتان في المجموعة نفسها إذا تحقّق **كل** ما يلي:

| الشرط | القيمة |
|---|---|
| نوع المحتوى | `metadata.type` ∈ `image` أو `video` — لا غير |
| الاتجاه | `value.type` واحد (كلاهما وارد أو كلاهما صادر) |
| التجاور | متتاليتان في القائمة المعروضة بلا شيء بينهما |
| الفجوة | ≤ **60 ثانية** بين كل رسالتين متجاورتين (لا بين أوّل المجموعة وآخرها) |
| غير محذوفة | `deleted_at` فارغ |
| أقلّ عدد | **2** — الصورة الواحدة تبقى فقاعة عادية |

وأي رسالة أخرى بينهما — نصّ أو **مستند** أو صوت أو تذكرة أو رسالة من الطرف الآخر — **تقطع** المجموعة. هذا سلوك واتساب نفسه: الألبوم صورٌ وفيديو لا مستندات.

`created_at` يصل نصّاً `'Y-m-d H:i:s'` بتوقيت المنشأة. **إن تعذّر تحليله فلا تضمّ**: الضمّ الخاطئ يُخفي رسالةً في شبكة لا يتوقّعها القارئ، والفصل الخاطئ لا يزيد على إبقاء الشكل كما كان.

### العرض

| العنصر | القاعدة |
|---|---|
| البلاطات | حتى **4**، والزائد يظهر «+N» على الرابعة |
| التعليق | أوّل تعليق غير فارغ في المجموعة — الإرسال الدفعي يُرفقه بالأولى وحدها |
| الوقت | وقت **آخر** رسالة في المجموعة |
| الحالة | **الأدنى يحكم**: مجموعة فيها رسالة لم تُقرأ ليست «مقروءة». و`failed` تعلو الكلّ |
| الفيديو | بلاطة بعلامة تشغيل — لا تفتحه في معاينة الصور |
| بلا ملف | `media` فارغ ⇒ بلاطة «المحتوى غير متاح»، وتبقى ضمن مجموعتها |

### ملاحظة عن الترتيب

كان إرسال الدفعة يُلقي وظيفةً لكل ملف على حدة، فيصل الملف البطيء (مستند كبير أو فيديو يُعاد ترميزه) **بين** الصور فيقطع تجاورها. صار الإرسال سلسلةً مرتَّبة، فترتيب الوصول = ترتيب اختيار المرسِل. هذا ما يجعل القاعدة أعلاه تعمل عند الطرفين.

### إن أردتم معرّف دفعة صريحاً

يمكن إضافة حقل في `metadata` (مثل `"batch": {"id": "...", "index": 0, "total": 5}`) فيصير الضمّ حتمياً بلا استنتاج. تكلفته: تغيير ما يُخزَّن، وتحديث هذا التقرير، وتعامل التطبيق مع الصفوف القديمة التي لا تحمله (يُستعمل معها الاستنتاج أعلاه). لم يُنفَّذ لأن واتساب نفسه لا يفعله — أخبرونا إن أردتموه.

---

## 10) إرسال الملفات من التطبيق
...
endpoint واحد للنص والملفات معًا.

قبل كده كان عندنا ٣ endpoints لنفس الغرض: `/send-msg` للنص، و `/send-media` للملفات (وكانت متقفلة أصلًا)، و `/send/media` اللي بتاخد link جاهز مش ملف. دلوقتي `/send-msg` بس، و `/send-media` **متقفلة عن قصد وبترجع 404**.

كل الملفات بتتبعت في request واحد، مش request لكل ملف.

### الحقول

| الحقل | النوع | مطلوب؟ | ملاحظات |
|---|---|---|---|
| `phone` | string | نعم | رقم العميل بصيغة دولية `+9665…`. لو مش موجود هننشئ الـ contact |
| `file[]` أو `files[]` | file (متعدد) | للملفات | الاسمين شغالين. كرّر نفس المفتاح لكل ملف. والمفرد بدون أقواس شغال برضو |
| `msg_uuid[]` | string (متعدد) | لا | **UUID لكل ملف** — مش UUID واحد للمجموعة كلها |
| `caption` أو `message` | string | لا | بيتحط على **أول ملف بس**. لو بعت الاتنين، `caption` هو اللي بيتنفذ |
| `type` | string | لا | `text` للنص. **وجود ملف أقوى منه**: لو بعت ملف و `type=text` هنبعت الملف والنص يبقى caption عليه |
| `first_name` / `last_name` | string | لا | بتتستخدم لما ننشئ contact جديد |

### الـ request بيروح فين

| الـ request | اللي بيحصل |
|---|---|
| من غير ملف و `type=text` | رسالة نص، زي ما هي |
| ملف واحد | نفس مسار الـ batch، بملف واحد |
| أكتر من ملف | chain واحد مرتّب |

### الخطوات اللي التطبيق بيعملها

1. **ابعت كل الملفات في request واحد**، بنفس الترتيب اللي اختاره المستخدم. الترتيب في `file[]` هو الترتيب اللي هيوصل بيه للعميل.
2. **ولّد UUID لكل ملف** وابعتهم في `msg_uuid[]` بنفس الترتيب (`msg_uuid[i]` بتخص `file[i]`). لو بعت UUID واحد لأكتر من ملف، أول ملف بينجح والباقي بيفشل — عمود `uuid` في الداتابيز unique. وأي ملف مش لاقي UUID، إحنا بنولّده.
3. **ابعت `caption` مرة واحدة بس**، متكررهاش على كل ملف.
4. **متستناش response ملف عشان تبعت اللي بعده.** السيرفر بيحطهم في chain مرتّب على queue اسمه `high`، فبيتبعتوا واحد ورا التاني بالترتيب. لو التطبيق بعتهم parallel، الترتيب هيبوظ والصور مش هتتجمع في album.
5. **الـ response معناه إن الرسالة دخلت الـ queue، مش إنها وصلت** (`data.queued: true`). حالة كل رسالة بتوصل في `logs` عن طريق Pusher أو الـ history زي أي رسالة تانية.
6. **اعرضهم كـ album** حسب قواعد القسم 9 (النوع image أو video، الفرق ≤ 60 ثانية، نفس الاتجاه، رسالتين على الأقل).

### الحدود

| الحد | القيمة |
|---|---|
| عدد الملفات في الـ request | `CHAT_MAX_BATCH_FILES` = **10** |
| الصور | مفيش حد لنوعها (السيرفر بيضغطها لحد واتساب 5MB) |
| الفيديو / الصوت | 16MB للملف |
| المستندات | 100MB للملف |
| مجموع الـ request | `post_max_size` على السيرفر — بيتحسب على **مجموع** الملفات مش ملف لوحده |

الامتدادات المقبولة:
- **صور**: `jpg, jpeg, png, gif, bmp, webp, svg, ico, heic, heif`
- **فيديو**: `mp4, avi, mov, wmv, flv, mkv, webm, 3gp, mpeg, mpg`
- **صوت**: `mp3, wav, ogg, aac, m4a, flac, wma, amr, opus`
- **مستندات**: `pdf, doc, docx, xls, xlsx, ppt, pptx, txt, csv, rtf, odt, ods, odp`

النوع بيتحدد من امتداد كل ملف لوحده، فينفع تبعت صورة وفيديو ومستند في نفس الـ request.

### الـ response لما ينجح
...
```json
{
  ...
  "data": {
    ...
    "files": 3,
    "contact_id": 91,
    "contact_uuid": "f66618e3-a3bc-43d5-a81f-2eed83ed77f2",
    "phone": "+966500000001"
  }
}
```

### الـ response لما يترفض

| الحالة | `statusCode` | إمتى |
|---|---|---|
| بيانات غلط | 400 | رقم غير صالح، امتداد مش مدعوم، أكتر من 10 ملفات — التفاصيل في `errors` |
| ملف أكبر من الحد | 400 | الـ `message` بيقول الحد بتاع النوع ده |
| الاشتراك مش فعّال | 403 | |
| واتساب مش متربط | 403 | |
| نافذة الـ 24 ساعة مقفولة | 422 | مفيش حاجة هتتبعت — استخدم template |
| فشل تجهيز الـ batch | 422 | الـ `message` فيه السبب |

الشكل ثابت في كل الردود: `statusCode` + `success` + `message`. ولو الـ request اترفض، يبقى **مفيش ولا ملف** دخل الـ queue — مش نص المجموعة اتبعت والباقي لأ.

### لو ملف فشل جوه المجموعة

الملفات اللي بعده بتكمل عادي. كل job في الـ chain بيكمل اللي بعده (`continueBatchOnFailure`)، والملف الفاشل بيظهر عند العميل بحالة `failed` وسببه في `logs`. والملف المؤقت على السيرفر مابيتمسحش غير بعد إرسال ناجح، عشان لو الـ job اتعاد يلاقي الملف موجود.

### مثال (cURL)

```bash
curl -X POST https://app.mnjz.net/api/v1/send-msg \
  -H "Authorization: Bearer $TOKEN" \
  -H "Accept: application/json" \
  -F "phone=+966500000001" \
  -F "caption=الفاتورة المرفقة" \
  -F "files[]=@first.jpg"  -F "msg_uuid[]=1b9d…" \
  -F "files[]=@second.jpg" -F "msg_uuid[]=2c0e…" \
  -F "files[]=@third.pdf"  -F "msg_uuid[]=3d1f…"
```

---

## 11) الملفات الكبيرة — الرفع بالـ chunks

> التعليمات الكاملة الجاهزة لمطور الجوال في ملف منفصل: `MOBILE_CHUNKED_UPLOAD.md`.

### المشكلة

Cloudflare بيقطع أي request بياخد أكتر من **125 ثانية**، بغض النظر عن حجمه. يعني لو سرعة الرفع 0.3MB/ثانية، أي ملف أكبر من ~40MB هيفشل مهما اتبعت لوحده. اللي بيحصل: 499 في لوج السيرفر، و 524 عند المستخدم، من غير أي رسالة خطأ — الـ progress bar بيقف في نصه وخلاص.

وفوق ده، `/send-msg` محكوم بـ `post_max_size` على **مجموع** الملفات.

الحل إن مفيش request ياخد وقت طويل: نبعت chunk صغير في كل request، فالوقت بيتحسب على الـ chunk مش على الملف كله. ده اللي الـ dashboard بيعمله من زمان، ودلوقتي بقى متاح للتطبيق.

### إمتى نستخدم إيه

| مجموع حجم الملفات | الطريق |
|---|---|
| **≤ 5MB** | `POST /api/v1/send-msg` — request واحد بكل الملفات (القسم 10) |
| **أكبر من 5MB** | chunks: كل ملف بيتقسم لقطع 5MB وبتتبعت على `POST /api/v1/chats/upload/chunk` |

الحساب على **المجموع** مش على عدد الملفات: 10 ملفات صغيرة في request واحد ممكن تعدي الـ 125 ثانية على نت ضعيف زي ما ملف كبير بيعديها.

### ينفع نستخدم الـ chunks دايمًا؟

آه، ينفع. الملف الصغير بيبقى chunk واحدة (`index=0`, `total=1`) بتتجمع وتتبعت في نفس الـ request — مفيش حالة خاصة ولا `if` زيادة في كود التطبيق.

بس ليها تمن، وده قرار محتاج تاخده:

| | `/send-msg` | chunks دايمًا |
|---|---|---|
| عدد الـ requests لـ 3 صور صغيرة | **1** | 3 |
| ضمان الترتيب | **chain السيرفر بيضمنه** | ترتيب رفع التطبيق، فلازم يرفع واحد ورا التاني |

لو المهم عندك إن كود الموبايل يبقى مسار واحد: استخدم الـ chunks دايمًا وارفع الملفات واحد ورا التاني. ولو المهم أقل عدد requests وأقوى ضمان لترتيب الـ album: استخدم `/send-msg` لما المجموع يكون ≤ 5MB.

### الـ endpoint

```
POST /api/v1/chats/upload/chunk       (multipart/form-data)
Authorization: Bearer <sanctum token>
```

| الحقل | النوع | مطلوب؟ | ملاحظات |
|---|---|---|---|
| `upload_id` | string ≤ 64 | نعم | **ID مستقل لكل ملف**. السيرفر بيبني منه اسم فولدر الـ chunks، فلو استخدمت نفس الـ ID لملفين هيتخلطوا ويبوظوا الاتنين |
| `index` | int ≥ 0 | نعم | رقم الـ chunk، بيبدأ من صفر |
| `total` | int ≥ 1 | نعم | عدد chunks الملف ده (≤ 4000) |
| `chunk` | file | نعم | البايتات نفسها، 5MB |
| `phone` | string | نعم | نفس رقم `/send-msg` |
| `file_name` | string ≤ 255 | نعم | الاسم كامل بامتداده — منه بنعرف النوع |
| `file_type` | string | لا | `image\|video\|audio\|document` — لو مبعتوش بنستنتجه من الامتداد |
| `caption` | string | لا | مع **أول ملف بس** |
| `msg_uuid` | string ≤ 64 | لا | UUID رسالة الملف ده |
| `first_name` / `last_name` | string | لا | لما ننشئ contact جديد |

### الخطوات اللي التطبيق بيعملها

1. احسب مجموع أحجام الملفات. لو ≤ 5MB استخدم `/send-msg` وخلاص.
2. لو أكبر: **لكل ملف لوحده، وواحد ورا التاني (مش parallel)**:
   - `uploadId = "<uuid>-<رقم الملف>"` — ID يخص الملف ده بس.
   - قسّم الملف لقطع 5MB: `total = ceil(size / 5MB)`.
   - ابعت كل chunk في request لوحده مع `index` و `total`.
   - `caption` مع أول ملف بس، و `msg_uuid` مختلف لكل ملف.
3. الـ response على أي chunk مش أخيرة: `data.completed = false` ومعاها `received` و `total` — استخدمهم في الـ progress bar.
4. الـ response على آخر chunk: `data.completed = true` — يعني الملف اتجمع والـ job اتبعت للـ queue. اتنقل للملف اللي بعده.
5. لو المستخدم لغى: `DELETE /api/v1/chats/upload/chunk` ومعاه `upload_id` — بيفضي المساحة على طول. (واللي بيتساب من غير إلغاء بيتمسح تلقائيًا بعد 24 ساعة.)

### قاعدتين مهمين

**chunk واحدة في الهوا في نفس اللحظة.** الـ chunks بتتقبل بأي ترتيب (التجميع بيمشي على `index` مش على وقت الوصول)، لكن لو بعتهم parallel ممكن requestين يشوفوا إن الـ chunks كملت في نفس اللحظة، فالملف يتجمع مرتين ويوصل العميل مرتين. ابعت chunk، استنى الـ response، ابعت اللي بعدها.

**إعادة المحاولة آمنة.** النت قطع في chunk؟ ابعت **نفس الـ index** تاني: بيتكتب مكان القديم ومابيتحسبش مرتين. وللاستئناف بعد انقطاع: نفس الـ `upload_id` وابعت اللي مجاش ليه response — رقم `received` في كل response بيقولك وصل كام.

### الـ responses

```jsonc
// chunk وصلت، الملف لسه مكملش
{"statusCode":200,"success":true,"message":null,
 "data":{"completed":false,"received":3,"total":12}}

// آخر chunk: الملف اتجمع والـ job اتبعت
{"statusCode":200,"success":true,"message":"Message sent successfully",
 "data":{"completed":true,"queued":true,"received":12,"total":12,
         "contact_id":91,"contact_uuid":"f666…","phone":"+966500000001"}}
```

| الحالة | `statusCode` |
|---|---|
| حقل ناقص أو امتداد مش مدعوم | 400 (التفاصيل في `errors`) |
| الملف بعد التجميع أكبر من حد نوعه | 400 |
| اشتراك مش فعّال / واتساب مش متربط | 403 |
| نافذة الـ 24 ساعة مقفولة | 422 |
| فشل التجميع | 500 |

كل الفحوصات (الاشتراك، الربط، النافذة، الامتداد) بتحصل **قبل ما نخزّن أي byte**، عشان مايحصلش إن المستخدم يرفع ملف كامل وبعدين يترفض.

### الحدود هنا

الملف بعد التجميع بيتقاس **بحد نوعه بس**: فيديو وصوت 16MB، مستندات 100MB، صور من غير حد (بتتضغط بعدين لحد واتساب 5MB). حد `post_max_size` مالوش دخل هنا — مفيش chunk بتوصله أصلًا.

### الترتيب والـ album

كل ملف بيبعت الـ job بتاعه لحظة ما chunks بتاعته تكمل، يعني ترتيب الوصول = ترتيب رفع التطبيق للملفات. عشان كده **ارفع واحد ورا التاني مش parallel**، وإلا الصور مش هتتجمع في album واحد (القسم 9). والمجموعة الصغيرة عن طريق `/send-msg` ضمانها أقوى: chain واحد السيرفر هو اللي بيتحكم فيه.

---

## 12) شكل الـ response بالضبط

كل الأمثلة هنا مأخوذة من ردود حقيقية للـ endpoints، لا من قراءة الكود.

### الغلاف المشترك

كل ردود `/send-msg` و `/chats/upload/chunk` لها نفس الغلاف:

| الحقل | النوع | ملاحظات |
|---|---|---|
| `statusCode` | int | يساوي HTTP status دائمًا |
| `success` | boolean | |
| `message` | string \| **null** | قد يكون `null` حتى عند النجاح |
| `data` | object \| **غائب** | غائب في كل ردود الخطأ (400 و 403 و 422 و 500) |
| `errors` | object \| غائب | في **400 فقط**. مفاتيحه أسماء الحقول، وقيمة كل مفتاح `string[]` |

يعني الـ model العام:
...
data class ApiResponse<T>(
    ...
    val data: T?,                  // null في الأخطاء
    val errors: Map<String, List<String>>? // 400 فقط
)
```

### `POST /api/v1/send-msg` — نجاح مع ملفات
...
|---|---|---|---|
| `data.queued` | boolean | لا | دائمًا `true` |
| `data.files` | int | لا | عدد الملفات التي دخلت الـ queue |
| `data.contact_id` | int | لا | |
| `data.contact_uuid` | string | لا | UUID |
| `data.phone` | string | لا | بصيغة E.164 |

الشكل نفسه لملف واحد، مع `files: 1`.

### `POST /api/v1/chats/upload/chunk` — قطعة لم تُكمل الملف

```json
{
  "statusCode": 200, "success": true, "message": null,
  "data": { "completed": false, "received": 1, "total": 2 }
}
```

### `POST /api/v1/chats/upload/chunk` — القطعة الأخيرة
...
| الحقل | النوع | nullable | ملاحظات |
|---|---|---|---|
| `data.completed` | boolean | لا | **المفتاح الوحيد الذي تحتاجه للتفريع** |
| `data.received` | int | لا | القطع التي وصلت |
| `data.total` | int | لا | كما أرسلته أنت |
| `data.queued` | boolean | يظهر عند `completed: true` فقط | |
| `data.contact_id` | int | يظهر عند `completed: true` فقط | |
| `data.contact_uuid` | string | يظهر عند `completed: true` فقط | |
| `data.phone` | string | يظهر عند `completed: true` فقط | |

الأسلم أن تُعرَّف الأربعة الأخيرة nullable وتُقرأ فقط عندما يكون `completed = true`.

### `DELETE /api/v1/chats/upload/chunk`

```json
{ "statusCode": 200, "success": true, "message": null, "data": { "discarded": true } }
```
...
مفاتيح `errors` هي أسماء الحقول. **ملاحظة**: حتى لو أرسلت الملفات باسم `files[]`، مفتاح الخطأ يكون `file` — لأن السيرفر يوحّد الاسمين إلى `file` قبل التحقق.

### 403 و 422 و 500 — بلا `data` وبلا `errors`
...
اعرض `message` كما هو للمستخدم — نصّه مترجم وجاهز للعرض.

### قاعدة التفريع

```
success == false            →  اعرض message، وإن كان statusCode == 400 فصّل من errors
data.completed == false     →  حدّث الـ progress bar فقط
data.completed == true      →  الملف اكتمل ودخل الـ queue، انتقل للملف التالي
```

`queued: true` تعني أن الرسالة دخلت الـ queue، لا أنها وصلت العميل. حالة التسليم تصل لاحقًا في `logs` عبر Pusher أو الـ history (القسم 4).

### ⚠️ رسالة النص — الشكل غير مستقر حاليًا

`POST /api/v1/send-msg` **بدون ملف** (`type=text`) يرجع `data` بثلاثة أشكال مختلفة حسب ما حدث داخليًا:

| الحالة | شكل `data` |
|---|---|
| الرسالة دخلت الـ queue | `{queued, contact_id, contact_uuid, phone}` |
| أُرسلت فورًا ونجحت | `{success: true, data: {messaging_product, contacts[], messages[], chat{…}}}` |
| أُرسلت فورًا وفشلت | `{success: false, message?}` — ومع ذلك الغلاف يقول `statusCode: 200` و `success: true` |

وتطبيق الجوال لا يرسل `tempMessageId`، فرسائله النصية تسلك المسار الفوري دائمًا — أي الشكلين الثاني والثالث.

هذا خلل حقيقي: لا يمكن كتابة parser واحد لثلاثة أشكال، والفشل يصل بحالة نجاح فلا يمكن اكتشافه.

**الإصلاح المقترح**: توحيد `data` لرسالة النص على نفس شكل الملفات:

```json
{ "statusCode": 200, "success": true, "message": "Message sent successfully",
  "data": { "queued": true, "contact_id": 1, "contact_uuid": "…", "phone": "+9665…" } }
```

وإرجاع 422 عند الفشل الحقيقي بدل 200. لم يُنفَّذ بعد لأنه يغيّر شكل رد يستهلكه التطبيق المنشور اليوم.
