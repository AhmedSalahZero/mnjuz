<template>
	<Dialog v-model:open="isOpen">
		<DialogContent>
			...
			<form id="work-schedule-form" v-if="selectedTeamMember" @submit.prevent="onSubmit"
				...
				<Accordion type="single" collapsible :class="cn('flex flex-col gap-y-3', {
					'pointer-events-none opacity-40': !hasWorkingHours,
				})
					">
					<AccordionItem v-for="(daySchedule, dayIndex) in schedule" :key="daySchedule.day"
						...
						<div class="flex items-stretch justify-between">
							<div class="flex items-center gap-x-3 py-4 pl-4">
								...
								<span v-if="getFirstValidShift(daySchedule) && daySchedule.active"
									class="text-xs text-muted-foreground">
									{{ formatTime(getFirstValidShift(daySchedule)?.start_time) }} -
									{{ formatTime(getFirstValidShift(daySchedule)?.end_time) }}
									<span v-if="daySchedule.shifts.length > 1"> (+{{ daySchedule.shifts.length - 1 }})
									</span>
								</span>
							</div>
							<AccordionTrigger
						...
						<AccordionContent class="pb-0"
							...
							<div class="flex flex-col gap-y-3 px-4 py-4">
								<div v-for="(shift, shiftIndex) in daySchedule.shifts" :key="shiftIndex"
									class="flex items-center gap-x-3">
									<MultiSelect class="flex-1" :name="`${daySchedule.day}-${shiftIndex}-start`"
										:options="getStartTimeOptions(dayIndex, shiftIndex)"
										v-model="schedule[dayIndex].shifts[shiftIndex].start_time" mode="single"
										:searchable="true" placeholder="Start time" />
									<span class="text-base font-medium">To</span>
									<MultiSelect class="flex-1" :name="`${daySchedule.day}-${shiftIndex}-end`"
										:options="getEndTimeOptions(dayIndex, shiftIndex)"
										v-model="schedule[dayIndex].shifts[shiftIndex].end_time" mode="single"
										:searchable="true" placeholder="End time" />
								...
								<Button type="button" variant="outline" size="sm" class="w-fit"
									:disabled="!canAddShift(daySchedule)" @click="addShift(dayIndex)">
			...
			<DialogFooter class="justify-end gap-2">
				...
				<Button type="submit" form="work-schedule-form" :disabled="form.processing">
