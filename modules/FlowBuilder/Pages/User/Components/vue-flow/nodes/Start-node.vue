...
const options = ref([
  { value: 'new_contact', label: trans('New contact') },
  { value: 'first_message', label: trans('First-Time Message Only') },
  { value: 'keywords', label: trans('Text contains specific keywords') }
]);
...
<template>
  <div class="max-w-[520px] rounded-sm border border-gray-200 bg-white p-3 shadow-md">
    <div class="flex flex-col gap-y-4">
      <div class="flex justify-between">
        <div class="flex items-center gap-x-2">
          ...
          <div class="flex flex-col gap-y-1">
            <p class="text-sm text-gray-500">{{ $t('Trigger') }}</p>
          </div>
        </div>
      </div>
      ...
      <span class="text-sm text-gray-500">{{ $t('Define the input parameters to trigger the workflow, to ensure that') }} <br> accurate information is captured in the conversation flow.</span>

      <div class="rounded-md bg-muted">
        <div class="mb-4">
          <label class="text-sm mb-2">{{ $t('Starting Step') }}</label>
          <FormSelect v-model="input1" :name="''" :type="'text'" :optionClassName="'h-32'"  :options="options" :class="'col-span-4'"/>
        </div>
        <div v-if="input1 === 'keywords'">
          <label class="text-sm mb-2"><span class="text-red-500">*</span> {{ $t('Trigger keywords') }}</label>
          <FormTextArea v-model="input2" :name="''" :placeholder="$t('Enter keywords separated by a comma')" :type="'text'" :class="'col-span-4'"/>
        </div>
        <div v-if="supportsTimeout" class="mt-4">
          <label class="text-sm mb-2">{{ $t('Timeout (minutes)') }}</label>
          <FormInput v-model="input3" :name="''" :type="'number'" :min="1" :placeholder="$t('e.g. 60')" :class="'col-span-4'"/>
          <p class="mt-1 text-xs text-gray-500">
            {{ $t('Leave empty to run only once. If set, the flow runs again when the contact writes after this many minutes of silence.') }}
          </p>
        </div>
      </div>
    </div>
