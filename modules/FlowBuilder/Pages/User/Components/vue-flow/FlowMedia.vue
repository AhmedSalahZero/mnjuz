...
<template>
	...
	<div class="flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
		...
		<input @change="handleFileChange($event)" v-if="props.type == 'video'" type="file" class="sr-only"
			accept=".mp4, .3gp" :id="'file-upload' + props.nodeId">
		<input @change="handleFileChange($event)" v-if="props.type == 'audio'" type="file" class="sr-only"
			accept=".aac, .mp3, .amr, .m4a" :id="'file-upload' + props.nodeId">
		<input @change="handleFileChange($event)" v-if="props.type == 'document'" type="file" class="sr-only"
			accept=".pdf, .txt, .xls, .xlsx, .doc, .docx, .ppt, .pptx" :id="'file-upload' + props.nodeId">
		<div class="text-center">
			<div v-if="modelValue.length == 0">
				<label for="file-upload">
					<svg v-if="props.type == 'image'" class="mx-auto h-12 w-12 text-gray-400 cursor-pointer"
						xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
						<path fill="currentColor" d="M14 9a1.5 1.5 0 1 1 3 0a1.5 1.5 0 0 1-3 0Z"></path>
					...
					<svg v-if="props.type == 'video'" class="mx-auto h-12 w-12 text-gray-400 cursor-pointer"
						xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
					...
					<svg v-if="props.type == 'audio'" class="mx-auto h-12 w-12 text-gray-400 cursor-pointer"
						xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
						<g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
							stroke-width="1.5" color="currentColor">
							<path
								d="M21 19.083v-3.166c0-1.769 0-2.653-.54-2.873s-1.176.405-2.447 1.656c-.662.651-1.047.791-1.971.791c-.82 0-1.229 0-1.523.194c-.604.396-.519 1.178-.519 1.815s-.085 1.419.519 1.815c.294.194.704.194 1.523.194c.924 0 1.309.14 1.97.791c1.272 1.251 1.908 1.877 2.448 1.656c.54-.22.54-1.104.54-2.873" />
							<path
								d="M12 22h-1.273c-3.26 0-4.892 0-6.024-.798a4.1 4.1 0 0 1-.855-.805C3 19.331 3 17.797 3 14.727v-2.545c0-2.963 0-4.445.469-5.628c.754-1.903 2.348-3.403 4.37-4.113C9.095 2 10.668 2 13.818 2c1.798 0 2.698 0 3.416.252c1.155.406 2.066 1.263 2.497 2.35C20 5.278 20 6.125 20 7.818V10" />
							<path
					...
					<svg v-if="props.type == 'document'" class="mx-auto h-12 w-12 text-gray-400 cursor-pointer"
						xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
