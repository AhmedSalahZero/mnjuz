...
| الحقل | القيم |
|---|---|
| `match_criteria` | `تطابق تام` أو `يحتوي` |
| `response_type` | **`نص` فقط في هذه النسخة** |

**هذان الحقلان يعودان بلغة الطلب.** الترجمة تتبع ترويسة `Accept-Language` التي يرسلها التطبيق: `ar` تُعيد `يحتوي` و`نص`، و`en` تُعيد `Contains` و`Text`. اعرضهما للمستخدم كما وصلا — لا تحتاج خريطة تسميات في التطبيق.

وعند `POST` و`PUT` يُقبل **ما أرسلناه إليك** وما سواه: `يحتوي` و`Contains` و`contains` كلّها تُحفظ قيمةً واحدة. فأعد ما استلمته بلا تحويل.
...
الصورة والصوت يحتاجان رفع ملف ولم يُدرجا بعد. الردود القديمة التي نوعها صورة أو صوت **تظهر في القائمة** بنوعها (`صورة` · `صوت`) و`response = null` — اعرضها للقراءة ولا تفتحها للتحرير.
