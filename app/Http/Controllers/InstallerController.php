...
class InstallerController extends BaseController
{
    ...
    public function runUpdate(Request $request){
        ...
        $migrateArtisanOutput = Artisan::output();
        $migrateSuccess = $migrateOutput === 0;
        \Log::info('Migration result', [
            'success' => $migrateSuccess,
            'exit_code' => $migrateOutput,
            'output' => $migrateArtisanOutput
        ]);
        ...
        if($migrateSuccess){
            ...
            if($dbUpdate){
                ...
                \Log::error('Seeder (dbUpdate) failed during update.', [
                    'output' => $seederOutput
                ]);
            }
        } else {
            \Log::error('Migration failed during update.', [
                'exit_code' => $migrateOutput,
                'output' => $migrateArtisanOutput
            ]);
            return response()->json([
        ...
        return response()->json([
            'statusCode' => 500,
            'message' => __('Update failed. Check logs for details.')
        ], 500);
    }
